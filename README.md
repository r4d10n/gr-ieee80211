...
**<font color=#5050f0>GR-WiFi</font>** : GNU Radio IEEE 802.11b/a/g/n/ac (WiFi 4/5) Transceiver
------
- **NEW:** 802.11b DSSS/CCK support (1, 2, 5.5, 11 Mbps) with both long and short preambles
- Support SISO and upto 2x2 **<font color=orange>SU-MIMO</font>** and **<font color=orange>MU-MIMO</font>**.
- Complete WiFi evolution coverage: 802.11b/a/g/n/ac (1 Mbps to 866 Mbps)
- Ubuntu 22.04 and GNU Radio 3.10 (recommended).
...
## 802.11b DSSS/CCK Support (NEW)
------

This module now includes complete IEEE 802.11b Direct Sequence Spread Spectrum (DSSS) and Complementary Code Keying (CCK) support, enabling communication with legacy 802.11b devices and providing robust low-rate connectivity.

### Supported Features

**Data Rates:**
- 1 Mbps (DBPSK with Barker code spreading)
- 2 Mbps (DQPSK with Barker code spreading)
- 5.5 Mbps (CCK with 4-bit encoding)
- 11 Mbps (CCK with 8-bit encoding)

**Preamble Types:**
- Long preamble (144 bits) - all rates
- Short preamble (72 bits) - 2, 5.5, 11 Mbps only

**PHY Layer Features:**
- Barker code correlation for packet detection
- Automatic rate detection from SIGNAL field
- Phase-locked loop (PLL) for carrier tracking
- PLCP header CRC-16 validation
- 7-bit LFSR scrambling (x^7 + x^4 + 1)
- Complete CCK chip generation and detection

### DSSS Blocks

**1. ieee80211_ppdu_prefixer**
- Message-driven block that adds PLCP preamble and header
- Configurable rate and preamble type (7 combinations)
- Automatic length calculation and CRC-16 generation

**2. ieee80211_ppdu_chip_mapper_bc**
- Converts bytes to complex DSSS/CCK chip sequences
- Supports all 802.11b modulation schemes
- Outputs 11 Msps sample rate

**3. ieee80211_chip_sync_c**
- Correlation-based packet detection and synchronization
- Demodulates all DSSS/CCK modes
- Configurable correlation threshold
- Outputs complete packets via message port

### Usage Example

```python
from gnuradio import gr, ieee80211, blocks

# Create a simple 802.11b transmitter
fg = gr.top_block()

# Create packet source (your MAC frames)
src = blocks.message_strobe(pmt.cons(pmt.PMT_NIL, pmt.make_u8vector(100, 0x42)), 1000)

# Add PLCP header (11 Mbps with long preamble)
prefixer = ieee80211.ppdu_prefixer(3)

# Map to chips
mapper = ieee80211.ppdu_chip_mapper_bc("packet_len")

# Connect to USRP or file sink
# ...

fg.msg_connect(src, 'out', prefixer, 'psdu_in')
fg.msg_connect(prefixer, 'ppdu_out', mapper, 'in')
```

### Integration Details

The 802.11b support has been integrated from [gr-wifi-dsss](https://github.com/r4d10n/gr-wifi-dsss) with full GNU Radio 3.10 compatibility:
- Modern C++17 with `std::mutex`, `std::shared_ptr`
- Pybind11 Python bindings
- YAML-based GRC block definitions
- Unified namespace: `gr::ieee80211`

### Credits

Original 802.11b implementation by Teng-Hui Huang (gr-wifi-dsss).
Integration and GNU Radio 3.10 port: 2025.
